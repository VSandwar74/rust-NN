...
fn parse_data(df: DataFrame) -> (Array2<f32>, usize, usize) {
    let ndarray: Array2<f32> = df.to_ndarray::<Float32Type>(IndexOrder::C).unwrap();
...
fn split_data(ndarray: Array2<f32>) -> (Array2<f32>, Array1<f32>, Array2<f32>, Array2<f32>, Array1<f32>, Array2<f32>, usize){
    ...
    let mut y_dev = &data_dev.slice(s![0, ..]);
    let mut x_dev = &data_dev.slice(s![1.., ..]);

    let _pre_t = &ndarray.slice(s![1000.., ..]);
    let data_train = _pre_t.t();
    let mut y_train = &data_train.slice(s![0, ..]);
    let mut x_train = &data_train.slice(s![1.., ..]);
...
fn get_predictions(a2: Array2<f32>) -> Array1<f32> {
    let mut predictions: Array1<f32> = Array1::<f32>::zeros(a2.shape()[1]);
